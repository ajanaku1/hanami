...
## 2026-09-13 — a music bed, because the noise could not be removed without damaging the voice

Every gap in the denoised take measures -72 to -91 dB, so the residual noise a listener still heard
was not in the gaps: it rides with the voice, at frequencies the voice occupies. Each tool that
removed meaningfully more of it also removed consonants — the wavelet pass took 21 dB of "hiss" and
21 dB of sibilance with it.

So the noise is masked rather than removed, which is worth stating plainly. A synthesised ambient
bed — four tones, a low drone in A with a fifth and octaves, drifting slowly, low-passed at 900 Hz —
is sidechained to the narration and sits 19 dB below speech, the usual depth for a narration bed,
and 36 dB above the room tone. At that level the noise is inaudible and the bed does not compete.

The bed is generated by ffmpeg from pure tones, so there is no third-party audio in the submission
and no licence to account for. `constants.ts` points at `voiceover-mixed.mp3`; pointing it back at
`voiceover.mp3` removes the music with no other change.
